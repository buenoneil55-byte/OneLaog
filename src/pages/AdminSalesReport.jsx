...
export default function AdminSalesReport() {
  ...
  return (
    <AdminLayout title="Sales Report">
      <div className="range-tabs" style={{ marginBottom: 16 }}>{ranges.map((r) => <button key={r.value} className={`range-tab ${range === r.value ? 'active' : ''}`} onClick={() => setRange(r.value)}>{r.label}</button>)}</div>
